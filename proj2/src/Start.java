...
public class Start {
	...
	public static int iPort = 18973;
	
	public static ArrayList<ProcTime> arrTimeProc = null;
	public static ArrayList<ArrayList<ProcTime>> arrRunTimeProc = null;
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		}else
		{
			...
			Process tRoot = Runtime.getRuntime().exec("ssh -o StrictHostKeyChecking=no "+Start.arrAddProc.get(0)+" cd " + path + "; java Start 0");
			tRoot.waitFor();
			return;
			// I am root
			//iSelf = 0;
		...
		try
		{
			File file= new File(Integer.toString(iSelf)+".txt");
		...
		arrTimeProc = new ArrayList<ProcTime>();
		...
		/*
		Iterator<ArrayList<ProcTime>> itrA = arrRunTimeProc.iterator();
		while(itrA.hasNext()){
			Iterator<ProcTime> itr = (Iterator<ProcTime>)(itrA.next()).iterator();
			while(itr.hasNext()){
				System.out.println(Integer.toString(itr.next().iProcId)+":"+Integer.toString(itr.next().iOriTime)+","+
						Integer.toString(itr.next().iUpTime));
			}
		}
		*/
	}
