...
public class ServerThread extends UnicastRemoteObject implements RW, Runnable {
	...
	public void sendRequest(String si, int iIndex, int iSleepTime) throws RemoteException {
		
		System.out.println("recv req from "+si+" at "+((int)System.currentTimeMillis()-Server.iLaunchTime));
        System.out.flush();
		try {
		...
		int iReqTime = (int)System.currentTimeMillis();
		System.out.println("req:"+si+" finish sleep at "+(iReqTime-Server.iLaunchTime));
        System.out.flush();
		/*
		try {
			semaReq.acquire();
		...
		if( arrClient.get(iIndex).getType() == 0 ){
			...
			Req r = new Req(si, iIndex, iReqTime , 0 , 3);
			...
			Req r = new Req(si, iIndex, iReqTime , 1 , 1);
	...
	public void updateFinishedTime(int iIndex, int iOpTime)	throws RemoteException {
		//System.out.println("updateFinishedTime("+iIndex+")");
		//System.out.flush();
		try {
			Thread.sleep(iOpTime);
		} catch (InterruptedException e1) {
			System.out.println(e1.toString());
            System.out.flush();
		}
		int iEndTime = (int)System.currentTimeMillis();
